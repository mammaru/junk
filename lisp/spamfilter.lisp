...
(defclass word-feature ()
  ((word
	...
	:documentation "word that express this feature")
   (spam-count
	:initarg :spam-count
	:accessor spam-count
	:initform 0
	:documentation "count of spams in which this feature present")
   (ham-count
	:initarg :ham-count
	:accessor ham-count
	:initform 0
	:documentation "count of hams in which this feature present")))
...
(defun fisher (probs number-of-probs)
  "Robinson's implementation of fisher score calculation"
  (inverse-chi-square
   ;(* -2 (log (reduce #'* probs)))
...
;; example to make new corpus
(defparameter *corpus* (make-array 1000 :adjustable t :fill-pointer 0))

(defun add-directory-to-corpus (dir type corpus)
  (dolist (filename (list-directory dir))
	(add-file-to-corpus filename type corpus)))
;; example to add mail/spam/ and mail/ham/ to corpus respectively
;(add-directory-to-corpus "mail/spam/" 'spam *corpus*)
;(add-directory-to-corpus "mail/ham/" 'ham *corpus*)

;;test function
(defun test-cllasifier (corpus testing-fraction)
  (clear-database)
...
(defun show-feature (feature)
  (with-slots (word ham-count spam-count) feature
	(format
	 t "~&~2t~a~30thams: ~5d; spams: ~5d;~,10tprobs: ~,f~%"
	 word ham-count spam-count (bayesian-spam-probability feature))))
(defun sorted-interesting (features)
  (sort (remove-if #'untraind-p features) #'< :key #'bayesian-spam-probability))
